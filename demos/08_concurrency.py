...
def demo_sync_vs_async() -> None:
    ...
    print("\n异步调用（并发）：")
    start = time.time()
    # - __`llm.ainvoke(q)`__：LangChain 的异步版 `invoke`。返回一个协程对象（coroutine），不会立即执行。
    # - __`asyncio.gather(*tasks)`__：并发的核心。让两个 LLM 请求同时发出，而不是串行等待。例如每个请求耗时 2 秒，串行需 4 秒，`gather` 并发只需约 2 秒。
